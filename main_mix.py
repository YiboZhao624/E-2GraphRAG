        ...
        with timer.timer("total"):
            with mp.Pool(processes=2) as pool:
                ...
                extract_args = (
                    ...
                    configs.get("language", "en"),
                    configs["extractor"],
                    configs["cluster"]["force_Reextract"],
                    configs["llm"],
                )
            ...
            for i, data_piece in enumerate(dataset):
                ...
                try:
                    ...
                    if "retriever" not in locals():
                        extractor_conf = configs["extractor"]
                        method = extractor_conf["method"]
                        extra_kwargs = {}
                        if method == "LLM":
                            extra_kwargs = {
                        ...
                        retriever = Retriever(
                            ...
                            index,
                            appearance_count,
                            load_nlp(extractor_conf["language"], method, **extra_kwargs),
                            **configs["retriever"]["kwargs"],
                        )
                    else:
                    ...
                    for j, qa_piece in enumerate(qa):
                        ...
                        try:
                            ...
                            if configs["rewrite_query_ret"] == True:
                                rewritten_question = llm.generate(Prompts["rewrite_query_zh_v2"].format(question=question), 
                                max_new_tokens=configs["llm"]["max_new_tokens"]).text
                                rewritten_question = rewritten_question.split("</think>")[1].strip()
                            else:
                                rewritten_question = question
                            logger.info(f"rewritten question: {rewritten_question}")

                            model_supplement = retriever.query(rewritten_question, **configs["retriever"]["kwargs"])
                        ...
                        if configs["dataset"]["dataset_name"] in ("NovelQA", "InfiniteChoice"):
                            ...
                            if configs["rewrite_query_qa"] == True:
                                rewritten_question = llm.generate(Prompts["rewrite_query_zh_v2"].format(question=question), 
                                max_new_tokens=configs["llm"]["max_new_tokens"]).text
                                rewritten_question = rewritten_question.split("</think>")[1].strip()
                            else:
                            ...
                            if configs.get("language", "en") == "zh":
                                input_text = Prompts["QA_prompt_answer_zh"].format(
                                    question=rewritten_question, evidence=model_supplement["chunks"]
                                )
                            else:
                            ...
                            output = llm.generate(input_text, max_new_tokens=configs["llm"]["max_new_tokens"])
