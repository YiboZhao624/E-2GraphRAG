...
def build_tree_task(args):
    llm_config, text, cache_folder, tokenizer_name, length, overlap, merge_num, language = args
    if os.path.exists(os.path.join(cache_folder, "tree.json")):
        return load_cache_summary(os.path.join(cache_folder, "tree.json")), -1
    try:
        llm_device = llm_config.get("llm_device", "cpu")
        ...
        # Process
        result, time_cost = build_tree(
            text, llm, cache_folder, tokenizer, length, overlap, merge_num, language
        )
