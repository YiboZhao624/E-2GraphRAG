...
class Retriever:
    ...
    def _merge_with_dense_results(self, primary_res:Dict[str, List[str]], query:str, top_k:int) -> Tuple[Dict[str, List[str]], Dict[str, List[str]]]:
        """
        Merge primary retrieval results with dense retrieval results, removing duplicate chunks.
        Returns a tuple of (merged_results, dense_results).
        """
        dense_res = {}
        if self.embedder is None or self.faiss_index is None:
            # dense retrieval unavailable, return primary only
            merged_res = {}
            seen_chunks = set()
            for key, chunk_list in (primary_res or {}).items():
                unique_list = []
                for chunk in chunk_list:
                    if chunk not in seen_chunks:
                        unique_list.append(chunk)
                        seen_chunks.add(chunk)
                if unique_list:
                    merged_res[key] = unique_list
            return merged_res, dense_res

        dense_res = self.dense_retrieval(query, top_k)
        ...
        for key, chunk_list in dense_res.items():
            merge_key = key if key else "dense"
            for chunk in chunk_list:
                if chunk not in seen_chunks:
                    merged_res.setdefault(merge_key, []).append(chunk)
                    seen_chunks.add(chunk)
    ...
    def query(self, query, **kwargs):
        ...
        if len(entities) == 0:
            ...
            merged_chunk_ids, dense_chunk_ids = self._merge_with_dense_results(chunk_ids, query, kwargs.get("max_chunk_setting", 25))
            result = {"chunks":self.format_res(merged_chunk_ids)}
            if kwargs.get("debug", True):
                supplement_info = self._build_supplement_info(merged_chunk_ids, entities, merged_chunk_ids, list(merged_chunk_ids.keys()), self._count_chunks(merged_chunk_ids), [])
                result.update(supplement_info)
                result["dense_chunk_ids"] = dense_chunk_ids
                result["retrieval_type"] = "Global Search + Dense Merge"
            return result
        ...
        if chunk_count == 0:          
            ...
            merged_chunk_ids, dense_chunk_ids = self._merge_with_dense_results(filtered_chunk_ids, query, kwargs.get("max_chunk_setting", 25))
            res_str = self.format_res(merged_chunk_ids)

            result = {"chunks":res_str}
            if kwargs.get("debug", True):
                supplement_info = self._build_supplement_info(merged_chunk_ids, entities, merged_chunk_ids, list(merged_chunk_ids.keys()), self._count_chunks(merged_chunk_ids), chunk_counts_history)
                result.update(supplement_info)
                result["dense_chunk_ids"] = dense_chunk_ids
                result["retrieval_type"] = "Occurrence Rerank + Dense Merge"
            return result
        ...
        if chunk_count != 0:
            ...
            merged_chunk_ids, dense_chunk_ids = self._merge_with_dense_results(local_res, query, kwargs.get("max_chunk_setting", 25))
            res_str = self.format_res(merged_chunk_ids)
            ...
            if kwargs.get("debug", True):
                ...
                result.update(supplement_info)
                result["dense_chunk_ids"] = dense_chunk_ids
                result["retrieval_type"] = f"Local + Dense Merge, Loop for {len(chunk_counts_history)-1} times"
            return result
        else:
            ...
            merged_chunk_ids, dense_chunk_ids = self._merge_with_dense_results(res_ids, query, kwargs.get("max_chunk_setting", 25))
            res_str = self.format_res(merged_chunk_ids)
            result = {"chunks":res_str}
            ...
            if kwargs.get("debug", True):
                ...
                result.update(supplement_info)
                result["dense_chunk_ids"] = dense_chunk_ids
                result["retrieval_type"] = f"EntityAware Filter + Dense Merge, Loop for {len(chunk_counts_history)-1} times"
            return result        
